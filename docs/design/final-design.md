...
**Multi-pseudo-thread scope.** The Phase 2b gate is satisfied iff (a) for every IO pseudo-thread `P` with a defined `attributed_delay_ratio(P)` (i.e., not `None`), `attributed_delay_ratio(P) ≥ 0.70`, **AND** (b) at least one IO pseudo-thread has a defined ratio. IO pseudo-threads whose ratio is `None` (no inbound non-return edges, or `raw_wait_ms` summing to zero) are **excluded** from condition (a) — they represent absence of IO traffic on that pseudo-thread, not a gate violation. Condition (b) is a **hard precondition**: a workload that produces no defined ratio at all (no IO pseudo-thread has any inbound non-return traffic) **fails** the gate, on the rationale that the Phase 2b gate is named "IO Attribution" and a workload with no observable IO has not exercised the gate's subject — a silent `pass` would mask cascade bugs that produce all-zero `raw_wait_ms` upstream (a class of regression that would otherwise be invisible to Phase 2b CI). Pure-CPU regression fixtures that intentionally exercise no IO must explicitly opt out of the Phase 2b gate at the test-runner level, not by relying on the gate to vacuously pass. This per-pseudo-thread evaluation with the (b) hard-precondition is forward-compatible with Phase 2c when Network and Softirq pseudo-threads enter the WFG: each newly-observed pseudo-thread must independently attribute correctly, with quiet pseudo-threads skipped from (a) but at least one observed pseudo-thread required by (b), preventing per-device bugs from being masked through aggregation. Defense-in-depth: commit-9 multi-IO fixtures (issue #38) MUST assert `attributed_delay_ratio` is `Some(r) where 0.70 ≤ r ≤ 1.0` rather than the looser `(0..=1.0)` range, so a magnitude regression in cascade or ratio computation surfaces independently of the gate verdict.
