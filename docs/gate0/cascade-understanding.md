...
3. **The Python validation script (`tests/fixtures/cascade_oracle.py`) covers linear/tree graphs only.** It implements the cascade from ADR-007 pseudocode but lacks sweep-line partition for overlapping outgoing edges (BUG-3). Phase 0's differential testing (#20) should restrict comparisons to non-overlapping topologies; complex graphs must rely on I-1/I-2 invariant assertions in the Rust implementation. (Originally `gate0/cascade-test/run_figure4.py`, preserved in tag `archived/gate0-prototypes`.)
